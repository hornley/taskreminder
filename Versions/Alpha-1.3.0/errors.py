...
if "Log" not in os.listdir(path1):
    os.mkdir(f"{path1}\Log")
if "Error" not in os.listdir(f"{path1}.\Log"):
    os.mkdir(f"{path1}\Log\Error")
...
class NotEmpty(Exception):
    def __init__(self, desc, message="is not empty!"):
        self.desc = desc
        self.message = message
