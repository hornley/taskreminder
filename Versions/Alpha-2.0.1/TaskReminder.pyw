...
icon_option = config.get("ICON", "icon_option")
if icon_option == '1':
    icon_path = config.get("ICON", "icon_path")
    if not icon_path.endswith(('.png', 'gif', '.ppm')) or icon_path == '' or not os.path.isfile(icon_path):
        config.set('ICON', 'icon_option', '0')
        _cnfg = open('config.txt', 'w')
        config.write(_cnfg)
        msg = 'Path is empty!' if icon_path == '' else 'Use a .png, .gif or .ppm image format!' if not icon_path.endswith(('.png', 'gif', '.ppm')) else 'File does not exist!'
        tkinter.messagebox.showerror(title='Icon path invalid', message=msg)
        quit()
appearance_mode = config.get("VISUAL", "mode")
...
def TaskAssigning():
    ...
    for i in newFC:
        if i in dictionary:
            ...
            try:
                xx = 2 + x * 5 # Gets the current value of x and assigns it to xx
...
def cfu_func():
    ...
    cfu_version = cfu[1]
    if version.replace(".", "") < cfu_version.replace(".", "") :
        link = cfu[0]
        return link, cfu_version
    return False
...
settings_icon = ctk.CTkImage(Image.open(f"{path}\settings_icon.png"))
...
def switch_event():
    ...
    config.set("VISUAL", "mode", ("dark" if appearance_mode == "light" else "light"))
    mode(x)
    settings_button.configure(fg_color=bar_color, hover_color=bar_color)
    windowLabel_Main.configure(text_color=window_label_colors)

switch_var = customtkinter.StringVar(value=("light" if appearance_mode == "light" else "dark"))
switch = ctk.CTkSwitch(master=app, text="", command=switch_event, variable=switch_var, onvalue="light", offvalue="dark", switch_width=30, switch_height=15)
switch.place(relx=0.25, rely=0.075, anchor=ctk.CENTER)
...
def settings(window):
    ...
    settings_window.geometry("1100+400")

    LabelTaskTitle = ctk.CTkLabel(master=settings_window, text='Settings', font=("Arial", 17), text_color=window_label_colors)
    ...
    topmost_label = ctk.CTkLabel(settings_window, text="Topmost", font=("Arial", 12)).place(relx=0.4, rely=0.48, anchor=ctk.CENTER)  # placement TBC
    ...
    buttonExit = ctk.CTkButton(master=settings_window, text="Close", command=settings_window.withdraw, width=15, height=10, font=("Arial", 8), fg_color=bar_color_t, hover_color=bar_color)
...
def Specfic_Task_Window(TaskTitle):
    ...
    Showing_Task.geometry("400x400+350+220")
    if icon_option == '1':
        Showing_Task.iconphoto(False, PhotoImage(file=icon_path))

    LabelTaskTitle = ctk.CTkLabel(master=Showing_Task, text=TaskTitle, font=("Arial", 17), text_color=window_label_colors)
    LabelTaskTitle.place(relx=0.5, rely=0.135, anchor=ctk.CENTER)
    ...
    settings_button = ctk.CTkButton(master=Showing_Task, text='', image=settings_icon, width=25, fg_color=bar_color_t, hover_color=bar_color,
                                    command=lambda: settings(Showing_Task)).place(relx=0.9, rely=0.1, anchor=ctk.CENTER)
    buttonExit = ctk.CTkButton(master=Showing_Task, text="Close", command=close, width=30, font=("Arial", button_size))
    buttonExit.place(relx=0.9, rely=0.935, anchor=ctk.CENTER)
    buttonDone = ctk.CTkButton(master=Showing_Task, text="Remove Subtask", command=done, width=30, font=("Arial", button_size))
    buttonDone.place(rely=0.935, relx=0.5, anchor=ctk.CENTER)
    buttonUpdate = ctk.CTkButton(master=Showing_Task, text="Update", command=update, width=30, font=("Arial", button_size))
    buttonUpdate.place(rely=0.935, relx=0.1, anchor=ctk.CENTER)
...
def Specific_Task():
    ...
    settings_button = ctk.CTkButton(master=Showing_Task, text='', image=settings_icon, width=25, fg_color=bar_color, hover_color=bar_color,
                                    command=lambda: settings(Showing_Task)).place(relx=0.9, rely=0.1, anchor=ctk.CENTER)
    button_task_title = ctk.CTkButton(master=Showing_Task, text="Choose", command=TaskTitleOption, width=50, font=("Arial", button_size))
    ...
    buttonExit = ctk.CTkButton(master=Showing_Task, text="Back", command=close, width=30, font=("Arial", button_size))
...
def All_Task():
    ...
    ctkBar = ctk.CTkScrollbar(master=All_Task, command=text.yview, height=300, width=16, border_spacing=2, fg_color=bar_color_t)
    ...
    settings_button = ctk.CTkButton(master=All_Task, text='', image=settings_icon, width=25, fg_color=bar_color_t, hover_color=bar_color,
    ...
    buttonExit = ctk.CTkButton(master=All_Task, text="Back", command=close, width=30, font=("Arial", button_size))
...
def Show_Task_Menu():
    ...
    settings_button = ctk.CTkButton(master=Show_Task_Menu, text='', image=settings_icon, width=25, fg_color=bar_color_t, hover_color=bar_color,
                                    command=lambda: settings(Show_Task_Menu)).place(relx=0.9, rely=0.1, anchor=ctk.CENTER)
    windowLabel_ShowTaskMenu = ctk.CTkLabel(master=Show_Task_Menu, text="Show Task Menu", font=("Arial", windowlabel_size), text_color=window_label_colors)
    windowLabel_ShowTaskMenu.place(relx=0.5, rely=0.2, anchor=ctk.CENTER)
    buttonSpecific = ctk.CTkButton(master=Show_Task_Menu, text="Specific Task", command=lambda: task("1"), width=70, font=("Arial", button_size))
    buttonSpecific.place(relx=0.5, rely=0.35, anchor=ctk.CENTER)
    buttonAllTask = ctk.CTkButton(master=Show_Task_Menu, text="All Tasks", command=lambda: task("2"), width=70, font=("Arial", button_size))
    buttonAllTask.place(relx=0.5, rely=0.5, anchor=ctk.CENTER)
    buttonExit = ctk.CTkButton(master=Show_Task_Menu, text="Back", command=close, width=30, font=("Arial", button_size))
...
def Modify_Task_Menu():
    ...
    OptionLabel = ctk.CTkLabel(master=Modify_Task_Menu, text='', font=("Arial", 15), text_color=option_label)
    OptionLabel.place(relx=0.5, rely=0.4, anchor=ctk.CENTER)

    due_date_Label = ctk.CTkLabel(master=Modify_Task_Menu, text='', font=("Arial", 15), text_color=option_label)
    ...
    def due_date(Option):
        ...
        day_om = ctk.CTkOptionMenu(due_dater, variable=ctk.StringVar(value='1'), width=75)
        year_om = ctk.CTkOptionMenu(due_dater, variable=ctk.StringVar(value=str(year)), width=75)

        def years(x):
            day_om.configure(variable=ctk.StringVar(value=x))
            year_label = ctk.CTkLabel(due_dater, text="Year:", font=("Roboto", 15)).place(relx=0.35, rely=0.6,
                                                                                            anchor=ctk.CENTER)
            year_om.place(relx=0.625, rely=0.6, anchor=ctk.CENTER)
            CTkScrollableDropdown(year_om, values=[str(i) for i in range(year, year + 11)])
            pass

        def dayers(x):
            month_om.configure(variable=ctk.StringVar(value=x))
            _d = calendar.monthrange(year, int(x))
            day_label = ctk.CTkLabel(due_dater, text="Day:", font=("Roboto", 15)).place(relx=0.35, rely=0.45,
                                                                                            anchor=ctk.CENTER)
            day_om.place(relx=0.625, rely=0.45, anchor=ctk.CENTER)
            CTkScrollableDropdown(day_om, values=[str(i) for i in range(1, _d[1] + 1)], command=years)
            pass

        month_label = ctk.CTkLabel(due_dater, text="Month:", font=("Roboto", 15)).place(relx=0.35, rely=0.3, anchor=ctk.CENTER)
        month_om = ctk.CTkOptionMenu(due_dater, variable=ctk.StringVar(value='1'), width=75)
        month_om.place(relx=0.625, rely=0.3, anchor=ctk.CENTER)
        CTkScrollableDropdown(month_om, values=[str(i) for i in range(1,13)], command=dayers)

        def grad_date():
            global datez
            global confirmation
            datez = f'{month_om.get()}/{day_om.get()}/{year_om.get()[2:]}'
    ...
    buttonChoose = ctk.CTkButton(master=Modify_Task_Menu, text="Confirm", command=OptionChose, width=30, font=("Arial", button_size))
    buttonChoose.place(relx=0.35, rely=0.8, anchor=ctk.CENTER)

    buttonReset = ctk.CTkButton(master=Modify_Task_Menu, text="Reset", command=OptionReset, width=30, font=("Arial", button_size))
    ...
    windowLabel_ShowTaskMenu = ctk.CTkLabel(master=Modify_Task_Menu, text="Modify Task Menu", font=("Arial", windowlabel_size), text_color=window_label_colors)
    windowLabel_ShowTaskMenu.place(relx=0.5, rely=0.2, anchor=ctk.CENTER)
    buttonExit = ctk.CTkButton(master=Modify_Task_Menu, text="Back", command=close, width=30, font=("Arial", button_size))
...
def Specfic_Task_Window_DUE(TaskTitle, SubTask):
    ...
    LabelTaskTitle = ctk.CTkLabel(master=Showing_Task, text=SubTask, font=("Arial", 17), text_color=window_label_colors)
    LabelTaskTitle.place(relx=0.5, rely=0.135, anchor=ctk.CENTER)
...
def hide_window():
    app.withdraw()
    app_settings()
    menu = (MenuItem('Show', show_window, default=True),
            MenuItem('Quit', quit_window),
            MenuItem('Check for almost Due', lambda: check_due(2))
            )
    image = Image.open(r"C:\Users\buend\Pictures\Genshin\123.ico")
    icon = Icon(name="Task Reminder", icon=image, menu=menu)
    icon.run()
...
class Due(Thread):
    ...
    def run(self):
        while self.running:
            due_amount = check_due(2)
            time.sleep(3600)  # Interval per check in seconds (3600s -> 1hr)
...
def main_settings(window):
    global changed
    changed = 0
    settings_window = ctk.CTkToplevel(window)
    settings_window.title('Settings')
    settings_window.attributes('-topmost', True)
    settings_window.geometry("300x300+1100+400")
    ...
    if window.attributes('-topmost'):
        topmost_var = customtkinter.StringVar(value="on")
    else:
        topmost_var = customtkinter.StringVar(value="off")
    ...
    def cfu():
        _x = cfu_func()
        if not _x:
            tkinter.messagebox.showinfo(title="Check for updates", message="There is no new update.")
            return
        link = _x[0]
        cfu_vers = _x[1]
        _m = tkinter.messagebox.askyesno(title="Check for updates", message=f"{cfu_vers} is available for download!\nDo you want to download the new version?")
        if _m:
    ...
    if backup_option == '1':
        backup_opt_var = customtkinter.StringVar(value="on")
    else:
        backup_opt_var = customtkinter.StringVar(value="off")

    def backup_opt():
        global changed
        changed = 1
        config.set('APP', 'backup', ('1' if backup_option != '1' else '0'))
        return

    # Reset after Modification

    if modify_reset == '1':
        mdf_rst_var = customtkinter.StringVar(value="on")
    else:
        mdf_rst_var = customtkinter.StringVar(value="off")

    def mdf_rst_mod():
        global changed
        changed = 1
        config.set('APP', 'modify_reset', ('1' if modify_reset != '1' else '0'))
        return

    # Minimizable

    if minimizable_option == '1':
        minimizable_var = customtkinter.StringVar(value="on")
    else:
        minimizable_var = customtkinter.StringVar(value="off")

    def minimizable():
        global changed
        changed = 1
        config.set('APP', 'minimizable', ('1' if minimizable_option != '1' else '0'))
        return

    # Icon

    if icon_option == '1':
        icon_var = customtkinter.StringVar(value="on")
    else:
        icon_var = customtkinter.StringVar(value="off")

    def icon():
        global changed
        changed = 1
        if icon_option != '1':
            config.set('ICON', 'icon_option', '1')
            settings_window.attributes('-topmost', False)
            _dia = filedialog.askopenfilename(title="Select a File", filetypes=(("png files","*.png"), ("gif files", "*.gif"), ("ppm files", "*.ppm")))
            _icon_path = _dia
            settings_window.attributes('-topmost', True)
            config.set('ICON', 'icon_path', _icon_path)
            return
        config.set('ICON', 'icon_option', '0')
        config.set('ICON', 'icon_path', '')
        return

    # Theme

    theme_var = "green" if theme_option == "green" else "blue" if theme_option == "blue" else "dark-blue"
    theme_om_var = customtkinter.StringVar(value=theme_var)

    def themers(themerse):
        global changed
        changed = 1
        config.set("VISUAL", "theme", "green" if themerse == "green" else "blue" if themerse == "blue" else "dark-blue")
        return
    ...
    def closex():
        if changed:
            _m = tkinter.messagebox.askokcancel(title="Restart to save changes", message="Changes will only be made after a restart.\nRestart now?")
            if _m:
                close()
    ...
    topmost_label = ctk.CTkLabel(settings_window, text="Topmost", font=("Arial", 12)).place(relx=0.4, rely=0.3, anchor=ctk.CENTER)  # placement TBC
    topmost_switch = ctk.CTkSwitch(master=settings_window, text="", command=topmost, width=20, height=10,
                                   variable=topmost_var, onvalue="on", offvalue="off").place(relx=0.75, rely=0.3, anchor=ctk.CENTER)  # placement TBC


    backup_opt_label = ctk.CTkLabel(settings_window, text="Backup", font=("Arial", 12)).place(relx=0.4, rely=0.4, anchor=ctk.CENTER)  # placement TBC
    backup_opt_switch = ctk.CTkSwitch(master=settings_window, text="", command=backup_opt, width=20, height=10,
                                   variable=backup_opt_var, onvalue="on", offvalue="off").place(relx=0.75, rely=0.4, anchor=ctk.CENTER)

    minimizable_label = ctk.CTkLabel(settings_window, text="Minimizable", font=("Arial", 12)).place(relx=0.4, rely=0.5, anchor=ctk.CENTER)  # placement TBC
    minimizable_switch = ctk.CTkSwitch(master=settings_window, text="", command=minimizable, width=20, height=10,
                                   variable=minimizable_var, onvalue="on", offvalue="off").place(relx=0.75, rely=0.5, anchor=ctk.CENTER)

    mdf_rst_label = ctk.CTkLabel(settings_window, text="Reset after modification", font=("Arial", 12)).place(relx=0.4, rely=0.6, anchor=ctk.CENTER)  # placement TBC
    mdf_rst_switch = ctk.CTkSwitch(master=settings_window, text="", command=mdf_rst_mod, width=20, height=10,
                                   variable=mdf_rst_var, onvalue="on", offvalue="off").place(relx=0.75, rely=0.6, anchor=ctk.CENTER)

    icon_label = ctk.CTkLabel(settings_window, text="Icon Option", font=("Arial", 12)).place(relx=0.4, rely=0.7, anchor=ctk.CENTER)  # placement TBC
    icon_switch = ctk.CTkSwitch(master=settings_window, text="", command=icon, width=20, height=10,
                                   variable=icon_var, onvalue="on", offvalue="off").place(relx=0.75, rely=0.7, anchor=ctk.CENTER)

    theme_label = ctk.CTkLabel(settings_window, text="Theme", font=("Arial", 12)).place(relx=0.4, rely=0.795,anchor=ctk.CENTER)
    theme_om = ctk.CTkOptionMenu(settings_window, variable=theme_om_var, command=themers, values=["green", "blue", "dark-blue"], font=("Arial", 11),width=65, height=23).place(relx=0.70, rely=0.8, anchor=ctk.CENTER)

    cfu_button = ctk.CTkButton(settings_window, text="Check for updates", width=30, command=cfu).place(relx=0.5, rely=0.9, anchor=ctk.CENTER)

    buttonExit = ctk.CTkButton(master=settings_window, text="Close", command=closex, width=15, height=10, font=("Arial", 8), fg_color=bar_color_t, hover_color=bar_color)
    buttonExit.place(relx=0.88, rely=0.92, anchor=ctk.CENTER)
    settings_window.protocol('WM_DELETE_WINDOW', closex)


settings_button = ctk.CTkButton(master=app, text='', image=settings_icon, width=15, height=10, fg_color=bar_color_t, hover_color=bar_color,
                                command=lambda: main_settings(app))
settings_button.place(relx=0.9, rely=0.08, anchor=ctk.CENTER)
windowVersionLabel_Main = ctk.CTkLabel(master=app, text=f"Version: {version}", font=("Roboto", 13))
windowVersionLabel_Main.place(relx=0.175, rely=0.95, anchor=ctk.CENTER)
windowLabel_Main = ctk.CTkLabel(master=app, text="Main Window", font=("Arial", windowlabel_size), text_color=window_label_colors)
windowLabel_Main.place(relx=0.5, rely=0.175, anchor=ctk.CENTER)
buttonShowTask_Main = ctk.CTkButton(master=app, text="Show", width=50, command=lambda: windowing(1), font=("Arial", button_size))
buttonShowTask_Main.place(relx=0.5, rely=0.35, anchor=ctk.CENTER)
buttonShowModify_Main = ctk.CTkButton(master=app, text="Modify", width=50, font=("Arial", button_size), command=lambda: windowing(2))
buttonShowModify_Main.place(relx=0.5, rely=0.5, anchor=ctk.CENTER)
buttonExit_Main = ctk.CTkButton(master=app, text="Exit", command=close, width=20, font=("Arial", button_size))
buttonExit_Main.place(relx=0.9, rely=0.9, anchor=ctk.CENTER)
if minimizable_option == '1':
